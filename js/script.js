...
popupButtons[0].addEventListener('click', ()=> {popup('hidden', 0)});
popupButtons[1].addEventListener('click', ()=> {popup('hidden', 0)});

function clearForm (){
    Name.value='', Email.value='', Phone.value='', Message.value='', grecaptcha.reset();
    Warning.innerHTML = '';
}
        ...
        axios.post('https://formsubmit.co/ajax/c649fc1ce0e6a95831be5af5e93d5957', {
            ...
            .then(response => {
                if(response.status == 200){
                    popup('hidden', 0);
                    popup('', 1);
                    // alert('Gracias por depositar su confianza en nosotros, lo estaremos contactando pronto');
                    clearForm();
                }
            })
            .catch(error => {
                popup('hidden', 0);
                popup('', 2);
                // alert('Ocurrio un error al enviar el mensaje, por favor vuelva a intentarlo');
                console.log(error);
            });
}
